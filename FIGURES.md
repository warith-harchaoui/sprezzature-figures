...
|------|--------|----------|-------------|
| `ablation_matrix` | make_ablation_matrix.py | Model evaluation | Ablation study: one row per model variant, a dot matrix of the components it keeps, and one metric panel per metric sharing that row axis. Use to show which parts of a model earn their place; hovering a row highlights it across every panel at once. |
...
| `diffusion_trajectory` | make_diffusion_trajectory.py | Dimensionality reduction | Animated point cloud carried through a diffusion model's noise schedule, forward then back, over a fixed nearest-neighbour graph so the structure being destroyed stays visible. Use to show what a noise schedule does to a manifold. |
| `donut` | make_donut.py | Composition | Pie chart with an open centre; each category's share of a whole is an arc's angular span, ordered largest to smallest, with a percentage label per wedge. Use for a small (2-6 category) part-of-whole breakdown: traffic by channel, budget by category, votes by candidate. **Count is not the only criterion: check the spread.** When the shares are close to each other, a donut is the worst choice available regardless of how few there are, because the eye compares neighbouring angles very poorly (Cleveland & McGill rank angle well below position and length). A real case that motivated this note: four segments at 27, 27, 24 and 22 percent rendered as a donut, where nothing could be ranked by looking and two wedges read identically. Sorted bars answer the same question instantly. Rule of thumb: if the largest share is less than about twice the smallest, use `bar` sorted descending, not `donut`. |
...
| `figure` | make_figure.py | n/a | Internal dispatcher; do not invoke directly. |
...
| `manifold_path` | make_manifold_path.py | 3-D surface | A walk across a two-coordinate objective surface, animated along the path it actually took, with the global optimum marked whether or not the walk reaches it. Use to show how an optimiser behaves, including where it stalls. |
...
| `pareto` | make_pareto.py | Quality / Ranking | Bar chart sorted descending with a cumulative-percentage line. Use to identify the "vital few" factors that account for the majority of an effect (80/20 rule). **Two preconditions, both checkable before choosing it.** The data must be the WHOLE population, not a top-N already truncated upstream: run on a `LIMIT 10`, the cumulative line reaches 100% of those ten and any headline about "the total" is false, because the tail it was meant to expose was cut off before the chart saw it. And the distribution must actually be concentrated: ten near-equal bars have no vital few to find, so the chart answers a question the data does not pose. Both were seen in one real chart, whose auto-generated title claimed "8 of 10 categories drive 82% of the total" about a top-10 subtotal. When either precondition fails, use `bar` sorted descending. |
...
| `quiver` | make_quiver.py | Vector field | One small rotated triangular arrow per grid point, direction and colour follow the local vector, size follows magnitude. Use for fluid-flow or wind-field snapshots, gradient fields, or any function assigning a direction and magnitude to every point in a plane. |
...
| `surface3d` | make_surface3d.py | 3-D surface | Static isometric 3-D surface for z = f(x, y): depth-sorted mesh quads shaded by height with a height legend. Hand-authored SVG. |
...
| `volcano` | make_volcano.py | Genomics / Statistics | Scatters effect size (log2 fold-change) against significance (-log10 p-value) for many simultaneous tests, dashed threshold lines isolate hits into the two upper corners. The standard chart for differential-expression results, or any large multiple-testing comparison. |
