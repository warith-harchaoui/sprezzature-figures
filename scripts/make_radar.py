...
from _svg import foreground_tip_css, fmt_compact, point_on_circle, svg_open, tooltip_bubble, xml_escape  # noqa: E402
...
def build_svg(
    ...
    # Every vertex bubble is collected here and drawn only once, all
    # together, as the last thing in the document (see the `parts.extend`
    # right before the legend): SVG has no z-index, so a bubble sitting
    # next to its own vertex would be covered by any vertex drawn afterward
    # (including a later series' dots). `tip_i` runs across every series so
    # each vertex and its bubble share a unique id (`hit-N`/`tip-N`).
    tips: List[str] = []
    tip_i = 0
    for s_idx, subject in enumerate(series):
        ...
        for i, (vx, vy) in enumerate(verts):
            ...
            parts.append(
                f'<g id="hit-{tip_i}" class="hit" tabindex="0" role="img" aria-label="{xml_escape(tip)}">'
