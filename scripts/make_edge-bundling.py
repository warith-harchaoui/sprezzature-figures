...
#: Row-record view of :data:`EDGES`, the shape the ``make_<kind>`` contract
#: asks for: one dict per module-dependency edge.
DEMO_DATA: List[Dict[str, Any]] = [
    {"source": u, "target": v} for u, v in EDGES
]
...
def make_edge_bundling(
    ...
    *,
    ...
    The six subsystems, their module leaves and the dependency edges
    (:data:`SUBSYSTEMS` / :data:`EDGES`) are baked into :func:`build_svg`,
    matching :data:`DEMO_DATA`; ``data`` is accepted for dispatcher parity
    but not threaded into the drawn graph, since the ring layout is
    computed once for this fixed subsystem/leaf hierarchy.
    ...
    out : Path, str, or None
        ...
        ``assets/svg-examples/edge-bundling.svg``.
    title : str, optional
