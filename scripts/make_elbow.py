...
from _svg import catmull_rom_beziers, fmt_compact, foreground_tip_css, svg_open, tooltip_bubble  # noqa: E402
...
def build_svg(
    ...
    *,
    ...
    dot_tips: list[str] = []
    for i, (k, v) in enumerate(zip(ks, inertia, strict=True)):
        cx, cy = sx(k), sy(v)
        tip = f"k = {fmt_compact(k)}: {fmt_compact(v)}"
        dot_fill = curve_deep if is_clear else muted
        p.append(
            f'<circle id="hit-{i}" class="hit" cx="{cx:.1f}" cy="{cy:.1f}" r="4.6" fill="{dot_fill}" '
        ...
        prev_v = v
    # Every dot's bubble drawn only after every dot, so a later point on the
    # curve can never cover an earlier one's bubble -- SVG has no z-index.
    p.extend(dot_tips)
