...
def build_svg(
    ...
    parts: List[str] = []
    parts.append(svg_open(width, height, "step-title", "step-desc"))
    ...
    for t, y in zip(ts, ys):
        ...
        tip = f"t={t:.0f}: {y:.0f}"
        parts.append(
            f'<circle class="pt" tabindex="0" cx="{cx:.1f}" cy="{cy:.1f}" r="4" fill="{COLOR_LINE}" '
    ...
    for t in ts:
        parts.append(
            f'<text x="{x_for(t):.1f}" y="{axis_y + 20:.1f}" font-size="11" font-family="{FONT_MONO}" '
